...
const LetterWritePage = () => {
	...
	const handleLetterSend = async () => {
		...
		if (act) {
			...
			// const response = await saveTextLetter(options, user.userId, title, content);
	...
	return (
		<>
			...
			<RowCenterWrapper>
				...
				>
					<LetterToggleButton category="write" act={act} setAct={!optionToggle && setAct} width="50%">
						편지지
					</LetterToggleButton>
					<LetterToggleButton category="draw" act={act} setAct={!optionToggle && setAct} width="50%">
						도화지
					</LetterToggleButton>
				</ContentBlock>
				<ContentBlock mWidth={SizeTypes.MOBILE_LETTER_WIDTH} optionToggle={optionToggle} ref={(el) => (unshowRef.current[1] = el)}>
					<Spacer act={act} />
				</ContentBlock>
				<ContentBlock
					...
					ref={wrapRef}
				>
					<LetterImg src={`${process.env.PUBLIC_URL}/assets/images/letter/${options.paper}.png`} />
					{act ? (
						<>
							<Input
								width="96%"
							...
							<LetterTextArea
								...
								ref={contentInput}
							/>
							<LetterProgressBar charCount={charCount} charCountWarning={charCountWarning} myRef={(el) => (unshowRef.current[2] = el)} />
						</>
					) : (
						<>
							<CanvasOptionBar canvasOptions={canvasOptions} setCanvasOptions={setCanvasOptions} />
							<LetterCanvasArea
								color="black"
								stroke="5"
				...
				{/* 편지지 선택 모달창 */}
				<Modal modalToggle={modalToggle} setModalToggle={setModalToggle} titleText={"편지지 선택"}>
					<PapersSelectGrid>
