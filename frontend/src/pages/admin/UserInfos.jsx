...
const UserInfos = () => {
	...
	const categoryList = ["번호", "닉네임", "연령대", "사용자 권한", "활성 여부", "정지 여부"];
	...
	useEffect(async () => {
		...
		if (response.status !== 200) {
			popWarningAlert("사용자 목록 조회 실패", "사용자 목록을 불러오던 중 문제가 발생했습니다.");
			return;
		}
	...
	const handlePermission = async (userId, role, e) => {
		...
		let tmp = [...itemList];
		tmp[userId - 1].role = role === RoleTypes.ADMIN ? RoleTypes.MEMBER : RoleTypes.ADMIN;
		setItemList(tmp);
	};
