...
import { accuseUser } from "./../../api/adminAPI";
import { popWarningAlert, popSuccessAlert } from "./../../utils/sweetAlert";
...
const Reports = () => {
	...
	/** 신고 처리 */
	const handleAccuse = async (idx, userId, e) => {
		if (tmpData[idx].isAccused) {
			popWarningAlert("신고 처리 실패", "이미 경고처리된 신고입니다.");
			return;
		}

		setLoading(true);
		const response = await accuseUser(userId);
		setLoading(false);
	...
	return (
		<>
			{modalToggle && (
				<Modal titleText={"편지 내용"} modalToggle={modalToggle} setModalToggle={setModalToggle}>
					<ModalContentWrapper>
					...
					<>
						{tmpData.map((item, index) => (
							<ContentLine key={index}>
								...
								<ContentElement flex={flexRate[1]}>{item.reportedTime}</ContentElement>
								...
								<ContentElement flex={flexRate[5]}>
									<Button bgColor="#49C288" width="90%" height="70%" onClick={() => handleModal(item.letterAccusationId)}>
										보기
									</Button>
								</ContentElement>
								<ContentElement flex={flexRate[6]}>
									<Button bgColor="#49C288" width="90%" height="70%" onClick={(e) => handleAccuse(item.letterAccusationId, item.reportedUserId, e)}>
										처리
									</Button>
								</ContentElement>
							</ContentLine>
