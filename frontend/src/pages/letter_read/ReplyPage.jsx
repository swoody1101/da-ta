...
const ReplyPage = () => {
  ...
  useEffect(() => {
    if (originLetter) {
      originLetter.letterInfo.imageLetterUrl
        ? setIsPicture(true)
        : setIsPicture(false);
      setShowLoading(true);
    } else {
  ...
  const handleLetterSend = async () => {
    ...
    const response = await saveReplyLetter(originLetter.letterInfo.letterId, {
      ...
      fontId:
        originLetter.letterInfo.fontId === null
          ? 0
          : originLetter.letterInfo.fontId,
    });
          ...
          <FlipFrontWrapper actFlip={flip}>
            <LetterImg
              src={`${process.env.PUBLIC_URL}/assets/images/letter/${
                LetterOptions.PAPERS[originLetter.letterInfo.backgroundId]
              }.png`}
            />
            <Input
              width="96%"
            ...
            <LetterProgressBar
              ...
              myRef={(el) => (unshowRef.current[0] = el)}
            />
          </FlipFrontWrapper>
          <FlipBackWrapper
            actFlip={flip}
            ref={(el) => (unshowRef.current[2] = el)}
          >
            <LetterImg
              src={`${process.env.PUBLIC_URL}/assets/images/letter/${
                LetterOptions.PAPERS[originLetter.letterInfo.backgroundId]
            ...
            {showLoading &&
              (isPicture ? (
                ...
                <>
                  <ReadLetterText
                    info={originLetter.letterInfo}
                    nickname={originLetter.writerNickname}
                  />
                </>
              ))}
          </FlipBackWrapper>
