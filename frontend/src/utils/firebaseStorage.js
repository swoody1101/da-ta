 ...
 * @param {string} path 파일이름 이전까지의 firebase storage 경로
 * @param {string} filename firebase storage에 저장된 파일 이름
 */
export const downloadFirebaseStorage = async (path, filename) => {
	const result = await getDownloadURL(ref(storage, `${path}${filename}`))
		.then((url) => url)
		.catch((error) => error);
	return result;
};
