...
const ReadLetterText = ({ info, nickname }) => {
	...
	const DateToString = (writtenDate) => {
		...
		return `${ToDate.getFullYear()}년 ${ToDate.getMonth() + 1}월 ${ToDate.getDate()}일`;
	};
	...
	return (
		<ContentBlock height={0} mWidth={SizeTypes.MOBILE_LETTER_WIDTH} mHeight={0} flexDirection="column" optionToggle={false} ref={wrapRef}>
			...
			<Container width="96%" height={SizeTypes.PC_TITLE_HEIGHT} padding="0.5rem 0 0 0" ref={(el) => (appearingRef.current[0] = el)}>
			...
			<Container width="96%" height={SizeTypes.PC_TITLE_HEIGHT} padding="0 0 0.5rem 0" ref={(el) => (appearingRef.current[1] = el)}>
				<LetterTitle width="96%" fontSize="1rem" fontWeight="bold">
					{`${DateToString(info.writtenDate)}, ${nickname}`}
				</LetterTitle>
			</Container>
			<LetterContent fontFamily={LetterOptions.FONTS[info.fontId]} ref={(el) => (appearingRef.current[2] = el)}>
				{info.content &&
					info.content.split("\n").map((line, index) => {
						return (
							<>
