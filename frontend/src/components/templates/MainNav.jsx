...
const MainNav = () => {
	...
	return (
		<Header headerShow={headerShow}>
			{headerMobileMode ? (
				<>
					<MobileLogo src={`${process.env.PUBLIC_URL}/assets/logo/data_logo.png`} height="4rem" onClick={() => navigate("/")} />
				</>
			) : (
				<>
			...
			{headerMobileMode ? (
				...
				<>
					<HeaderContents>
						...
						<HeaderContent onClick={() => navigate("/read")}>편지 읽기</HeaderContent>
						<HeaderContent onClick={() => navigate("/mypage")}>마이페이지</HeaderContent>
					</HeaderContents>
					<AuthWrapper>
						{isLogin ? (
							<>
								<Button hoverBgOpacity="0.3" fontSize="1.2rem" height="3rem" width="11rem" margin="0 -3.5rem 0 0 " onClick={handleLogout}>
									로그아웃
								</Button>
							</>
						) : (
